...
Example: python monitor.py -c "python process_data_entry.py" -i 1 -p ".*?\.csv" -v
...
def watched_file_changed(file_type, interval):
    ...
    for root, dirs, files in os.walk(os.path.curdir):
        for fname in files:
            try:
                filematched=re.match(file_type, fname)
                if filematched:             #re.match(file_type, fname):
                    if file_changed(os.path.join(root, fname), interval):
                        logging.info("File modified: " + filematched.group(0))
                        return True
            except re.error:
